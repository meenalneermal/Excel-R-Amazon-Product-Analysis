...
trans=pickle.load(open('vector.pkl','rb'))
...
def week():
	if request.method == 'POST':
	  	...
	  	camera=pd.DataFrame()
	  	camera['REVIEWS']=total_review
	  	camera['RATINGS']=total_rating
	  	camera['DATE']=total_date
	  	camera['RATINGS']=camera['RATINGS'].replace({'1':'NEGATIVE','2':'NEGATIVE','3':'POSITIVE','4':'POSITIVE','5':'POSITIVE'})
	  	lemmit= WordNetLemmatizer()
	  	tokens=[]
	  	for i in range(len(camera)):
	  		review = re.sub('[^a-zA-Z]', ' ', camera['REVIEWS'][i])
	  	...
	  	for x in range(len(tokens)):
	  		micro=camera.replace(to_replace=camera['REVIEWS'][x],value=tokens[x])
	  	Data=pd.DataFrame()
	  	...
	  	for i in range(1,8):
	  		...
	  		if previous_date[0]==str(0):
	  			...
	  			pass
	  		x=camera.loc[camera['DATE']==previous_date,'REVIEWS']
	  		z=[]
	  		if len(x)>0:
	  			z=stack_bal.predict(trans.transform(camera.loc[micro['DATE']==previous_date,'REVIEWS']))
	  			for y in z:
	  				week_data.append(y)
	  			for a in x:
